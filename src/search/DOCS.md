...
- `dev/tests/test_query_logger.py`'s engine mocks must expose `.search_with_reason(query, language, max_results) -> (results, empty_reason, diagnosis)` — `_engine_with_timing` calls that, not `.search()`. Fixed 2026-08-20 (the file's one shared mock helper, `_make_mock_engine`, set `.search` and was removed along with the drift it caused — see `_make_mock_engine_with_reason`, now the file's only engine-mock helper). Extended to a 3-tuple as of the diagnosis-snapshot milestone — `diagnosis` defaults to `None`, matching every engine's success path and `openalex`/`scholar`'s permanent shape.
- **`engine_stats[name]` (both `engine_run` and `workflow_summary` records) carries a `"diagnosis"` key alongside `"status"`, as of the diagnosis-snapshot milestone.** For the 7 browser engines it is a `dict` of raw DOM/JS facts (`marker`/`title`/`url`/`ready_state` plus engine-specific extras — see `engines/DOCS.md`'s Gotchas for the exact field contract) whenever `status` is a non-`None` empty_reason (`EMPTY_BLOCK`, `EMPTY_CONSENT`, `EMPTY_CONCURRENT_RACE`, `EMPTY_NO_CONTAINER`, `EMPTY_NO_RESULTS`), and `None` on `OK` and on any status not produced by `search_with_reason` itself (`TIMEOUT_*`, `ERROR_*`, `RATE_SKIP` — these come from `_engine_with_timing`'s own exception/rate-limiter handling, which has no tab to diagnose). Always `None` for `openalex`/`scholar` (no diagnosis mechanism built for HTTP-API engines). This is ADDITIVE ONLY — no existing `status`/EMPTY sub-status value changed; HTTP status capture is a separate, later milestone.
