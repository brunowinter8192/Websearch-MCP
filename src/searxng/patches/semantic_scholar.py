...
def get_ui_version() -> str:
    ...
    if not ret_val:
        ...
        # store session cookies (s2Exp, tid) alongside UI version — same TTL
        cookies = dict(resp.cookies)
        if cookies:
            CACHE.set("S2-Cookies", value=cookies, expire=300)
        logger.debug("X-S2-UI-Version: %s", ret_val)
    return ret_val


def get_session_cookies() -> dict:
    cookies = CACHE.get("S2-Cookies")
...
def request(query: str, params: "OnlineParams") -> None:
    ...
    params["cookies"] = get_session_cookies()
    params["json"] = {
        "queryString": query,
        "page": params["pageno"],
        "pageSize": 10,
