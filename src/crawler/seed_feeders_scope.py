...
# to "/", and drop the fragment. Query string and any non-root trailing slash are left exactly
# as given — either can denote a genuinely different document, and this feeder's worst case for
# over-merging is a seed that is never fetched at all (see DOCS.md Gotchas for the full boundary).
...
# Dedup key: a normalized URL with its host collapsed through _host_key. Scheme, path, and
# query stay exactly as normalize_url produced them — see normalize_url's own docstring for
# why those are kept distinct.
def _dedup_key(normalized_url: str) -> str:
    ...
    return urlunparse((parsed.scheme, netloc_key, parsed.path, "", parsed.query, ""))
